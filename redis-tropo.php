...
/*
 * A simple PHP script to subscribe to a Redis channel and play back any 
 *  information recieved using TTS.
 */
...
$redis->subscribe("tropo.speech");

say("Welcome to the Redhis and socket I O speech test.");

do {
	$response = $redis->getResponse();
	$message = $response[2];
	if($message == "1") {
		say("I am ready to speak.");
	}
	else {
		say($message);	
	}	
